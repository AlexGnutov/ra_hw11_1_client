...
function EditForm(props) {
    ...
    const handleChange = event => {
        ...
        dispatch(changeServiceField(name, value)); // dispatching object
    }

    const handleSubmit = event => {
        event.preventDefault();
        fieldsetRef.current.disabled = true;
        saveService(dispatch, {id: parseInt(id, 10), ...item}, onSuccess); // call AC and dispatch from it
    }

    useEffect(() => {
        loadService(dispatch, id); // call AC and dispatch from it
    }, [dispatch, id]);
